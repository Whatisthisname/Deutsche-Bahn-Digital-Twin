#!/usr/bin/env python3
"""
Converts the original station-based CSV to our internal journey-based CSV with separate departure/arrival events. Also does some renumbering of stations and filters out junk journeys
"""
    ...
    for ride_id, ride_rows in rides.items():
        ...
        for current_station, next_station in itertools.pairwise(ride_rows):
            if current_station.actual_departure_time > next_station.actual_arrival_time:
                ...
                break
            if (
                current_station.actual_arrival_time is not None
                and current_station.actual_departure_time is not None
            ):
